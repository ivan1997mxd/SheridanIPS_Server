...
class Matrix(ABC):
    ...
    def __init__(self, access_points, size: int = 5):
        ...
        self.__Vector_Dict = dict()             # type: Dict[int, Dict[int, float]]
        self.__zones = [1, 2, 3, 4, 5]
    ...
    @property
    def measured_zones_and_vectors(self) -> Tuple[int, Dict[int, List[float]]]:
        for measured_zone, vector in self.__Vector_Dict.items():
            yield measured_zone, vector

    @property
    def vectors(self) -> Tuple[int, Dict[int, float]]:
        for measured_zone, vector in self.__Vector_Dict.items():
            yield measured_zone, vector
    ...
    # region Getters
    def get_value(self, measured_zone: int, actual_zone: int) -> float:
        return self.__Vector_Dict[measured_zone][actual_zone]

    def get_vector(self, measured_zone: int) -> Dict[int, float]:
        return self.__Vector_Dict[measured_zone]

    def get_vector_sum(self, measured_zone: int) -> float:
        return sum(self.__Vector_Dict[measured_zone])
    # endregion

    # region Setters
    def set_value(self, measured_zone: int, actual_zone: int, value: float) -> None:
        self.__Vector_Dict[measured_zone][actual_zone] = value
    # endregion

    # region Incrementer
    def increment_value(self, measured_zone: int, actual_zone: int, increment_value: Union[int, float] = 1) -> None:
    ...
    @staticmethod
    def scalar_vector(vector: Dict[int, float], scalar: float) -> Dict[int, float]:
