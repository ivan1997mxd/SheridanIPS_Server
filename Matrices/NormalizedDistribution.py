...
class NormalizedDistribtuion(Matrix):
    ...
    def __init__(self, matrix: Matrix, combine_ids: bool = False):
        super(NormalizedDistribtuion, self).__init__(access_points=matrix.access_points)
    ...
    #JC - cross-relation
    @classmethod
    def __get_vector_error(cls, measured_zone: int, vector: Dict[int, float]) -> float:
        m = max(vector.values())
        a = vector[measured_zone]
        # if NormalizedDistribtuion.error_mode == "DGN":
        #     return 1 - a #vector[measured_zone]
        # elif NormalizedDistribtuion.error_mode == "MAX":
        return 1 - a #vector[measured_zone] #max(vector.values())

        raise NotImplementedError("Error mode {} is unknown".format(NormalizedDistribtuion.error_mode))

    @staticmethod
    def get_vector_success(measured_zone: int, vector: Dict[int, float]) -> float:
...
def build_normalized_distribution(probability_distribution: ProbabilityDistribution) -> NormalizedDistribtuion:
    return build_normalized_distributions([probability_distribution])[0]


def build_normalized_distributions(probability_distributions: List[ProbabilityDistribution]) -> List[NormalizedDistribtuion]:
    return [NormalizedDistribtuion(p) for p in probability_distributions]


def sort_matrices(matrix_list: List[NormalizedDistribtuion] = None,
                  matrix_tuples: Iterable[Tuple[Matrix, Matrix]] = None
                  ) -> Union[List[Tuple[ProbabilityDistribution, NormalizedDistribtuion]], None]:
